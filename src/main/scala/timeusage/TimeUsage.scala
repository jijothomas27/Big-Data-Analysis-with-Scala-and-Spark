...
object TimeUsage {
  ...
  def row(line: List[String]): Row =
    Row.fromSeq((line.head :: line.tail.map(_.toDouble)).toSeq)
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    def classify (activity: String):String  =
      activities.getOrElse(activity.take(5),activities.getOrElse(activity.take(3),"other"))

    val groups = columnNames.map(column => new Column(column))
                  .groupBy (column => classify(column.toString()))


    println(groups.getOrElse("other",Nil).length)

    (groups.getOrElse("primary needs",Nil),groups.getOrElse("work",Nil),groups.getOrElse("other",Nil))
  }
  ...
  def timeUsageSummary(
    ...
    val workingStatusProjection: Column = when(df("telfs").between(1,2),"working").otherwise("not working") as "working"
  ...
  def timeUsageGroupedSqlQuery(viewName: String): String =
    s"""
      ...
      |sum(primaryNeeds)/count(*) as primaryNeeds,
      |sum(work)/count(*) as work,
    ...
    import org.apache.spark.sql.expressions.scalalang.typed
      summed
        .groupByKey( row => (row.working, row.sex, row.age))
            .agg(avg('primaryNeeds).as[Double]).as("primaryNeeds")
            .agg(avg('work).as[Double]).as("work")
            .agg(avg('other).as[Double]).as("other")
          .orderBy('working, 'sex, 'age)
            .as[TimeUsageRow]
  }
}
